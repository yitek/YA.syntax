        ...
        var assert = function (actual, expected, msg, paths) {
            if (actual === expected) {
                if (!Unittest.hiddenSteps && msg && !paths) {
                    _this._$logger.info(msg.replace(/\{actual\}/g, JSON.stringify(actual)).replace(/\{expected\}/g, JSON.stringify(expected)));
                }
                return;
            }
            ...
            if (t === "object") {
                ...
                if (!actual)
                    throw new Error(paths.join(".") + "不应为空.");
